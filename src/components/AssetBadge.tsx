...
/**
 * Exact "S" glyph geometry from Saturn's real logo kit (Circle/Square USDat.svg,
 * Circle/Square sUSDat.svg) — background + glyph share one color, sitting on a
 * white square window. Reused for all four assets, recolored per token.
 */
export function SaturnMark({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 200 200" className="h-full w-full" aria-hidden="true">
      <rect width="200" height="200" rx="40" fill={color} />
      <path
        d="M148.193 52.5H52.5V148.193H148.193V52.5Z"
        fill="white"
      />
      <path
      ...
      <path
        ...
        fill={color}
      />
      <path d="M66.7258 102.768H32.6748V107.897H66.7258V102.768Z" fill={color} />
      <path
        d="M134.248 91.3716H168.156V96.5008H134.248V91.3716Z"
        fill={color}
      />
    </svg>
  );
...
export function AssetIcon({
  ...
  return (
    <span className={`inline-block shrink-0 overflow-hidden rounded ${className}`}>
      <SaturnMark color={meta.color} />
    </span>
  );
}

export function AssetBadge({ asset }: { asset: AssetSymbol }) {
  const meta = assetMeta[asset];
  return (
